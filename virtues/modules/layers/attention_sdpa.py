...
class MHAwithPosEmb(nn.Module):
    ...
    def forward(
        ...
        attn_mask: Optional[torch.Tensor] = None,  # (L, S) or (B, L, S) bool keep-mask, or additive
        ...
        Routing:
            - If cu_seq_len/max_seq_len are provided AND FlashAttention is available, use the
              FA varlen path. Otherwise, all calls go through the SDPA path.
            - SDPA path supports key_padding_mask (B, S), attn_mask (L,S)/(B,L,S), or both.
        """

        use_flash = (cu_seq_len is not None or max_seq_len is not None)
        if use_flash and not _FLASH_ATTN_AVAILABLE:
            raise RuntimeError(
                "cu_seq_len/max_seq_len were provided but flash_attn is not installed. "
                "Pass key_padding_mask or attn_mask instead, or install flash_attn."
            )
        if use_flash and (key_padding_mask is not None or attn_mask is not None):
            raise ValueError(
                "Provide either (cu_seq_len, max_seq_len) OR key_padding_mask/attn_mask, not both."
            )
        ...
        # Branch A: SDPA/manual path (default; also handles the no-mask case)
        if not use_flash:
            combined_mask = self._build_sdpa_mask(
                key_padding_mask=key_padding_mask,
                attn_mask=attn_mask,
                B=B, L=L, S=S, device=Q.device, dtype=Q.dtype,
            )
            ...
            if not return_attention:
                # Fast path: PyTorch SDPA (mem-efficient backend works on V100/T4/RTX 20xx)
        ...
        out = attn_out.reshape(B, -1, self.num_heads * self.head_dim)
        return self.W_o(out)
    ...
    def _build_sdpa_mask(
        ...
        """
        Combine key_padding_mask (B, S) and attn_mask into a single mask broadcastable
        to (B, H, L, S) for SDPA.

        Conventions:
            key_padding_mask: bool, True = PAD (will be masked out), shape (B, S).
                              Or float additive mask of shape (B, S).
            attn_mask:        bool keep-mask (True = keep) of shape (L, S), (B, L, S),
                              or (B, 1, L, S). Or float additive mask of compatible shape.
        Returns:
            None if both inputs are None;
            a bool keep-mask if only attn_mask is given as bool;
            an additive float mask otherwise.
        """
        if key_padding_mask is None and attn_mask is None:
            return None

        def _kpm_to_additive(kpm):
            if kpm.dtype == torch.bool:
                add = torch.zeros(B, 1, 1, S, device=device, dtype=dtype)
                return add.masked_fill(kpm[:, None, None, :], float("-inf"))
            return kpm[:, None, None, :].to(dtype)

        def _expand_attn_mask(am):
            if am.dim() == 2:
                am = am[None, None, :, :]
            elif am.dim() == 3:
                am = am[:, None, :, :]
            elif am.dim() != 4:
                raise ValueError(f"attn_mask must have 2/3/4 dims, got {am.dim()}")
            return am

        if attn_mask is None:
            return _kpm_to_additive(key_padding_mask)

        am = _expand_attn_mask(attn_mask)

        if key_padding_mask is None:
            if am.dtype == torch.bool:
                return am  # SDPA accepts bool keep-masks directly
            return am.to(dtype)

        # Both provided → combine as additive
        kpm_add = _kpm_to_additive(key_padding_mask)
        if am.dtype == torch.bool:
            am_add = torch.zeros_like(am, dtype=dtype)
            am_add = am_add.masked_fill(~am, float("-inf"))
        else:
            am_add = am.to(dtype)
        return am_add + kpm_add