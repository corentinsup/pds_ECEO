...
class TerraMeshViTDecoder(nn.Module):
    ...
    def forward_list(self, x: List[torch.Tensor], 
        ...
        for (x_i, ps_i) in zip(x, patch_summary_tokens):
            ...
            x_copy[curr_idx: curr_idx + num_channels, 0] = ps_i.expand(num_channels, H, W, D) # patch summary token broadcasted to all channels of the sample
            x_copy[curr_idx: curr_idx + num_channels, 1] = x_i # full spatial grid, masked position = masked token
